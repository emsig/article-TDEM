...
%
Looking carefully at these three \new{key} points results in much smaller
...
%
\\[1cm] \textbf{Keywords}: Controlled source electromagnetics (CSEM); Numerical
modelling; Fourier analysis.
\end{abstract}
...
line-relaxation and semicoarsening \citep{ECCFD.06.Jonsthovel} \new{are
...
responses}\old{precise results obtained from \empymod}. The blue circles
indicate the actually computed responses and the black dots the frequencies
which are interpolated or set to zero. The resulting time-domain response has a
relative error of less than 1\,\% everywhere except for very early times.
...
{(a) Same as Figure \ref{fig:GUI-FFTLog-lin}, but on a logarithmic scale. (b)
Using DLF instead of FFTLog, but computing the same frequenc\old{ies}\new{y
range} as for the FFTLog. The resulting time domain response has comparable
accuracy (compare the error to the error in Figure \ref{fig:GUI-FFTLog-lin}).
}
%
...
necessary offset range, the \old{less}\new{fewer} frequencies are needed.
...
work on a logarithmic scale we cannot choose 0\,Hz, but instead take
$10^{-100}$\,Hz). In the case of the DLF method we also have to interpolate in
between the computed frequencies, for which we found it better to use a cubic
spline. As can be seen from Figures \ref{fig:GUI-FFTLog-lin} and
\ref{fig:GUI-DLF-lin}, using the FFTLog or the DLF with the same actually
computed frequencies results in very similar responses.
...
of cells which fulfils these criteria. The \new{implemented} multigrid method
...
\plot*{fullspace}{width=\textwidth}{
  ...
  computed responses\old{ with \emg3d}, the black dots are the interpolated
...
result, where the dashed black line is the result \old{from
\emg3d}\new{obtained by transformation of the frequency-domain response}, on
...
\citep{CAG.15.Cockett}\old{. \simpeg}\new{, which} is a framework for modelling
...
framework}\old{\simpeg}.

The model was discretised with $100\times100\times100\,$m cells in the survey
domain $D_\mr{s}$. For the \old{\simpeg}\new{time-domain} model, 14 cells in
...
\old{\emg3d}\new{frequency-domain model}, the mesh is generated
...
responses and the relative error using the semi-analytical result\old{obtained
from \empymod}, and in (b) the responses including the target.
%
\plot{3d-result}{width=\textwidth}{
  Responses for the model outlined in Figure~\ref{fig:3d-model} using
  \new{time-domain}\old{\simpeg} and \new{frequency-domain
  computations}\old{\emg3d}, and for the layered background also \new{the
  semi-analytical result}\old{\empymod}. In the lower plot of (a) the relative
  error (\%) \new{is shown} in comparison to \new{the semi-analytical
  result}\old{\empymod is shown}, and in (b) the normalized difference (\%)
...
a relative error of a few percents at most; the result \new{obtained through
transformation}\old{for \emg3d} seems to be better at early times. We cannot
compare the errors for the response that includes the target for lack of an
analytical solution. The \new{semi-analytical} background model \old{from
\empymod }is only included to show that there is a significant response from
the target. We therefore show the normalized difference (\old{ND}\new{NRMSD})
between the two \old{3D codes}\new{responses $R_1$ and $R_2$} as a percentage,
where \old{$\mr{ND}= 50|(\simpeg-\emg3d)/(\simpeg+\emg3d)|$.}
\new{$\mr{NRMSD~(\%)} = 200|R_1 - R_2|/(|R_1| + |R_2|)$.} The
\old{ND}\new{NRMSD} between the two codes is below 1\,\% everywhere except for
early times.
...
\num{466752} cells \old{for \simpeg}\new{in the time domain} and meshes between
\num{102400} and \num{614400} \old{for \emg3d}\new{in the frequency domain}.
Because the memory of our test machine was not large enough for the direct
solver of \simpeg, the last model had to be run on a cluster. In this scenario,
\emg3d was about eight times faster (single thread).
...
\new{The values for lower frequencies can be interpolated using PCHIP knowing
