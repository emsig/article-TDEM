...
\textbf{Keywords}: CSEM, Fourier transforms, 3D modelling.
}
...
frequencies are ideally equally spaced on a logarithmic scale, as is the case
for, e.g., the digital linear filter method or the logarithmic fast Fourier
transform. With careful selection of frequencies and interpolation, 15 to 20
frequencies are enough to cover a wide range of offsets in one-dimensional test
models.
%
The gridding should be frequency-dependent, which is generally accomplished by
making it a function of skin depth. Optimising for the least number of required
cells for a given frequency should be combined with optimising for
computational speed.
%
Looking carefully at these three points results in much faster computation
times with speedup-factors of ten and more over previous results. A
frequency-domain computation of time-domain data can therefore be competitive
to time-domain solvers if the required frequencies and their corresponding
grids are carefully chosen.
%
\end{abstract}
...
non-seismic tools in exploration geophysics, not just in hydrocarbon
exploration \citep{GEO.10.Constable}, but also in the search for sulfides
\citep{GRL.19.Gehrmann}, water \citep{GEO.05.Pedersen}, or geothermal sources
\citep{WGC.15.Girard}. While in the deep marine environment current sources
with few frequencies are used, transient measurements are more common in the
shallow marine environment and on land. One of the main reasons is the
dominance of the airwave in shallow marine and terrestrial measurements, which
can be better separated in the time domain.

Acquired CSEM data are often analysed (modelled and inverted for) in their
...
responses directly in the required domain, either frequency or time. However,
...
Time-domain modelling with a frequency-domain code requires the computation of
a relatively wide range of frequencies for the Fourier transform. However, the
required computation grids for low frequencies (in our case in the order of
0.001\,Hz) and high frequencies (around 100\,Hz) are hugely different. Low
frequencies can be computed on a coarser grid, but they require a much larger
domain in order to avoid boundary effects. High frequencies, on the other hand,
require denser gridding, but they are much more limited in reach. An adaptive
gridding scheme is therefore required. We build our approach upon
\cite{GEO.07.Plessix}, who presented such an adaptive gridding for
...
which is the distance after with the amplitude of the electromagnetic field is
...
100\,$\Omega$\,m 1\,km below the seafloor in a background of 1\,$\Omega$\,m.
...
$f_\mathrm{max}$ is chosen appropriately, the importance of higher frequencies
...
interpolation by assuming a zero imaginary response at a very low frequency,
and we then use PCHIP to interpolate the missing frequencies. In the case of
the DLF method we also have to interpolate in between the computed frequencies,
for which we found it better to use a cubic spline. As can be seen from Figures
...
requirement by the direct solver would otherwise be too high.
...
We have shown that time-domain CSEM modelling with a frequency-domain code can
be competitive given a robust frequency-domain solver, a frequency-dependent
gridding function that minimizes the required cells, and a Fourier transform
that works on a logarithmic scale. Fast layered modelling can be used to design
the required frequency range, as the Fourier transform does not know about the
dimensionality of the underlying model. Twenty frequencies or less are usually
sufficient for a wide range of offsets. We believe that our proposed
improvements to the previously published methods make time-domain CSEM
modelling with a frequency-domain code even more attractive than it was already
before.
