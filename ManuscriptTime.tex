...
Fourier transforms are used, which can make it prohibitively expensive\deleted{
in comparison with a time-domain computation}. The speed of
\replaced{such}{time-domain} modelling \replaced{which is followed by a
transform}{with frequency-domain computation} is defined by three key points:
solver, method and implementation of the Fourier transformation, and gridding.
%
...
\deleted{frequency-domain} computation \replaced{followed by transformation}{of
time-domain data} can therefore be competitive to \replaced{computation in the
required }{time-}domain \deleted{solvers} if the required \replaced{evaluation
points}{frequencies} and the corresponding grids are carefully chosen.
%
\end{summary}
...
through transformation}{for \emg3d} seems to be better at early times.
\added{The reason is probably the implemented backward Euler in the time-domain
code which has an error of order one in time.} We cannot compare the errors for
the response that includes the target for lack of an analytical solution. The
\added{1D} background model \deleted{from \empymod }is only included to show
that there is a significant response from the target. We therefore show the
normalized difference (\replaced{NRMSD}{ND}) between the two
\replaced{responses $R_1$ and $R_2$}{3D codes} as a percentage, where
