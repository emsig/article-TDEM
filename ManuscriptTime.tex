...
comprehensive overviews. \replaced{Focusing on r}{R}ecent publications
\added{after these review articles} span\added{s} the width of numerical
...
\mbox{\cite{GEO.04.Commer, GEO.07.Maao}}); implicit schemes \mbox{\citep[e.g.,
][]{GEO.04.Haber}}, and matrix exponentials and Lanczos reduction schemes
\mbox{\cite[e.g., ][]{RS.94.Druskin}} with the Fourier transform of
...
\mbox{\citep{AEU.77.Weiland}} in the frequency domain to show our method it is
important to note that our ideas can be used with any solver, it is not bound
...
gridding exists as well \mbox{\citep[e.g., ][]{RS.06.Commer}}.} In the next
section, we briefly review the methodology as introduced by
...
hard to interpolate\added{ and we therefore set to zero}. Figure
\ref{fig:GUI-DLF-lin} is the same as \ref{fig:GUI-FFTLog-lin}, but transformed
with the DLF method applying the 81-point sine-cosine filter from
...
the above parameters were used to model the responses at offsets $r=1.5,3,6$,
and 12\,km. \added{We only need to compute 50\,\% of the required frequencies
in the case of the FFTLog, and only 14\,\% in the case of the DLF.}
%
\begin{figure*}
  \centering
...
required cells. This is generally best done in a frequency and conductivity
dependent manner.} To go from the model grid to the computational grid, we use
the volume-averaging technique on logarithmic resistivities, as used in
...
interpolated.} \deleted{We believe that our proposed improvements to the
...
from the frequency to the time domain. However, it could equally be applied to
the transform from the time to the frequency domain and to other methods with
similar characteristics. \added{We believe that our proposed improvements to
the previously published methods makes simulating results in one domain
obtained through computations in the other domain followed by a transformation
a viable alternative. The methodology is relatively simple to implement in any
code.}
