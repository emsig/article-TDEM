\documentclass[onecolumn,extra,referee,camera]{gji}
% \documentclass[onecolumn,extra,camera]{gji}
% \documentclass[twocolumn,extra,camera]{gji}
\usepackage{timet}
...
\usepackage{ifthen}
\usepackage[draft, addedmarkup=bf]{changes}
...
frequencies to cover a wide range of offsets\deleted{in one-dimensional test
models}.
%
...
both domains\deleted{, as it is done, for instance, in \simpeg (Cockett et al.,
...
parallelization over frequencies is straight forward.} For the semi-analytical
...
though we limit our analysis here to frequency-to-time transformation, most of
the arguments are reversible and applicable to time-to-frequency
...
convert the extrapolation of missing frequencies into \added{an }interpolation.
...
$\rho=0.3\,\ohmm$ ($\rho = \sigma^{-1}$) and a 100\,m thick target of
100\,\ohmm at 1\,km below the seafloor in a background of 1\,\ohmm. The source
is 50\,m above the seafloor, the receiver is on the seafloor, and the response
is the inline $x$-directed $E$-field. The left subplot shows the imaginary part
in the frequency domain and the right plot the corresponding impulse response
in the time domain. The red lines are the \replaced{semi-analytical
responses}{precise results obtained from \empymod}. The blue circles indicate
the actually computed responses and the black dots the frequencies which are
...
\begin{figure*}
  ...
  frequenc\replaced{y range}{ies} as for the FFTLog. The resulting time domain
...
frequencies could be left out. \added{But even so we only need to compute
50\,\% of the required frequencies in the case of the FFTLog, and only 14\,\%
in the case of the DLF.} \replaced{W}{However, w}ith this frequency-selection
...
\begin{figure*}
  ...
  \caption{Normalized (a) frequency- and (b) time-domain responses for offsets
    ...
    interpolated. The \replaced{grey}{black} curves are the analytical
    responses.}
...
stretching $\alpha_\mr{c}$. In \replaced{the presented examples}{our scheme} we
...
of cells which fulfils these criteria. The \added{implemented} multigrid method
...
\added{It is important to note that this is our implementation of an adaptive
grid, but there are certainly other possibilities. The relevant point for fast
computations is that the adaptive gridding tries to minimize the number of
required cells while being fine enough and at the same time large enough to
give accurate results. This is generally best done in a frequency and
...
\begin{minipage}{\twidth}
  ...
  \begin{tabular}{S[table-format=2.4]rrrr}
    ...
    \midrule
    %
    20.0    &\replaced{ 4}{ 3} & 80$\times$24$\times$24 & 20 & 1.26 \\
    12.6    &\replaced{ 8}{ 6} & 96$\times$32$\times$32 & 20 & 1.17 \\
     7.98   &\replaced{ 8}{ 8} & 96$\times$32$\times$32 & 20 & 1.20 \\
     5.03   &\replaced{ 8}{ 7} & 96$\times$32$\times$32 & 20 & 1.23 \\
     3.18   &\replaced{ 7}{ 7} & 80$\times$32$\times$32 & 24 & 1.23 \\
     2.00   &\replaced{ 7}{ 5} & 80$\times$32$\times$32 & 30 & 1.21 \\
     1.26   &\replaced{ 5}{ 4} & 64$\times$32$\times$32 & 37 & 1.21 \\
     0.798  &\replaced{ 5}{ 5} & 64$\times$32$\times$32 & 40 & 1.23 \\
     0.503  &\replaced{ 5}{ 5} & 64$\times$32$\times$32 & 40 & 1.26 \\
     0.318  &\replaced{ 5}{ 5} & 64$\times$32$\times$32 & 40 & 1.28 \\
     0.200  &\replaced{ 8}{10} & 64$\times$40$\times$40 & 40 & 1.27 \\
     0.126  &\replaced{ 8}{10} & 64$\times$40$\times$40 & 40 & 1.30 \\
     0.0798 &\replaced{10}{14} & 80$\times$40$\times$40 & 40 & 1.26 \\
     0.0503 &\replaced{11}{15} & 80$\times$40$\times$40 & 40 & 1.28 \\
    %
    \bottomrule
  \end{tabular}
\end{minipage}
...
\begin{minipage}{\twidth}
  ...
  \begin{tabular}{S[table-format=2.5]rrrr}
    ...
    \midrule
    %
    31.8     & \replaced{16}{10} & 128$\times$40$\times$40 & 100 & 1.36 \\
    20.0     & \replaced{17}{ 9} & 128$\times$40$\times$40 & 100 & 1.36 \\
    12.6     & \replaced{17}{ 9} & 128$\times$40$\times$40 & 100 & 1.36 \\
     7.98    & \replaced{17}{17} & 128$\times$40$\times$40 & 100 & 1.36 \\
     5.03    & \replaced{17}{21} & 128$\times$40$\times$40 & 100 & 1.44 \\
     3.18    & \replaced{17}{20} & 128$\times$40$\times$40 & 100 & 1.48 \\
     2.00    & \replaced{17}{18} & 128$\times$40$\times$40 & 100 & 1.49 \\
     1.26    & \replaced{20}{22} & 128$\times$40$\times$48 & 100 & 1.36 \\
     0.798   & \replaced{24}{26} & 128$\times$40$\times$48 & 100 & 1.36 \\
     0.503   & \replaced{27}{29} & 128$\times$40$\times$48 & 100 & 1.36 \\
     0.318   & \replaced{27}{42} & 128$\times$40$\times$48 & 100 & 1.36 \\
     0.200   & \replaced{34}{41} & 128$\times$40$\times$48 & 100 & 1.38 \\
     0.126   & \replaced{34}{48} & 128$\times$40$\times$48 & 100 & 1.40 \\
     0.0798  & \replaced{37}{54} & 128$\times$40$\times$48 & 100 & 1.41 \\
     0.0503  & \replaced{37}{59} & 128$\times$40$\times$48 & 100 & 1.44 \\
     0.0318  & \replaced{44}{74} & 128$\times$40$\times$48 & 100 & 1.44 \\
     0.0200  & \replaced{44}{81} & 128$\times$40$\times$48 & 100 & 1.47 \\
     0.0126  & \replaced{37}{96} & 128$\times$40$\times$48 & 100 & 1.48 \\
     0.00798 & \replaced{43}{93} & 128$\times$40$\times$48 & 100 & 1.49 \\
    %
    \bottomrule
  \end{tabular}
\end{minipage}
...
response was chosen as this is the response currently implemented in
\replaced{the framework}{\simpeg}.
...
a relative error of a few percents at most; the result \replaced{obtained
through transformation}{for \emg3d} seems to be better at early times. We
cannot compare the errors for the response that includes the target for lack of
an analytical solution. The \added{semi-analytical} background model
\deleted{from \empymod }is only included to show that there is a significant
response from the target. We therefore show the normalized difference
(\replaced{NRMSD}{ND}) between the two \replaced{responses $R_1$ and $R_2$}{3D
codes} as a percentage, where \replaced{$\mr{NRMSD~(\%)} = 200|R_1 -
R_2|/(|R_1| + |R_2|)$.}{$\mr{ND}= 50|(\simpeg-\emg3d)/(\simpeg+\emg3d)|$.} The
\replaced{NRMSD}{ND} between the two codes is below 1\,\% everywhere except for
early times.

\added{¡TODO CHANGE THIS LAST PARAGRAPH!}
...
\num{466752} cells \replaced{in the time domain}{for \simpeg} and meshes
between \num{102400} and \num{614400} \replaced{in the frequency domain}{for
\emg3d}. Because the memory of our test machine was not large enough for the
...
interpolation. The actual transform can be made with either the DLF method or
FFTLog, where the latter one requires usually much fewer frequencies to be
interpolated.} We believe that our proposed improvements to the previously
