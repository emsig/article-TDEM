...
\added{TODO: Add citation to R.R. (2021) regarding RA and speed-up; limitation
to switch-on/-off.}

\added{TODO: Add notion about preference for f-domain for IP problems}
...
equations in an isotropic, homogeneous medium is given by \citep[e.g.,][
...
time-domain result. The same applies for the reverse operation, the fewer times
required, the quicker we obtain frequency-domain results. Even though we limit
our analysis here to frequency-to-time transformations, most of the arguments
are reversible and applicable to time-to-frequency transformations. We decided
to use a regular spacing of frequencies on a log-scale, rather than an adaptive
...
is therefore often tailored to a single offset. Also, it allows for
straightforward parallelization over frequencies, which is not completely
possible with an adaptive scheme.
...
The shown example model is a marine scenario with 1\,km water depth of
...
hard to interpolate and we therefore set it to zero. Figure
\ref{fig:GUI-DLF-lin} is the same as Figure \ref{fig:GUI-FFTLog-lin}, but
...
\begin{figure*}
  ...
  accuracy (compare the error to the error in Figure
...
\begin{figure*}
  ...
  \caption{Normalized (a) frequency- and (b) time-domain responses for offsets
    ...
    \ref{fig:GUI-FFTLog-lin}. The coloured symbols are the actually computed
...
\ref{fig:GUI-FFTLog-lin} with Figure \ref{fig:GUI-DLF-lin}, using the FFTLog or
...
\begin{figure*}
  ...
  \caption{(a) Frequency- and (b) time-domain results for the homogeneous space
    ...
    and the dashed black line the obtained time-domain response. The errors are
    clipped for values smaller than 0.1\,\% and bigger than 100\,\%.}
  \label{fig:fullspace}
\end{figure*}
%
...
\begin{figure*}
  ...
  \caption{Responses for the model outlined in Figure~\ref{fig:3d-model} using
    ...
    and in (b) the normalized difference (\%) between the two 3D codes. The
    results of these plots are clipped for values smaller than 0.1\,\% and
    bigger than 10\,\%.}
  \label{fig:3d-result}
\end{figure*}
%
