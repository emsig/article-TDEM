...
scenarios and acquisition layouts which have all different implications. A
well-known case is for instance the land or shallow marine case versus the deep
marine case, where in the former case a dominating airwave has to be accurately
modelled which can can be completely ignored in the latter case. Recent areas
of particular interest in time-domain modelling that pose numerical challenges
are, for instance, simulating the fields through steel-cased wells
\mbox{\citep{CAG.19.Heagy}} or the effects of induced polarization
\mbox{\citep{EXP.20.Kang}}. The former is challenging because of the high
conductivity contrasts requiring very detailed meshing, the latter is
...
GEO.07.Maao})\replaced{ and}{;} implicit schemes \citep[e.g.,
][]{GEO.04.Haber}\added{ have a complexity of $O(n^4)$}, \deleted{and} matrix
exponentials and Lanczos reduction schemes \cite[e.g., ][]{RS.94.Druskin}
\added{have a complexity of $O(n^4\sqrt{\log n})$}\replaced{, and}{with the}
Fourier transform of frequency-domain solutions \added{have a complexity $O(n_f
n^3)$}; \added{$n$ is the number of grid points in each direction, and $n_f$
the number of frequencies}. Their conclusion was that the Fourier transform
method can be favourable if the number of required frequencies is small
relative to the number of grid points in each coordinate. The conditions for
fast computation of time-domain data with a frequency-domain code are: a
...
computation time. \replaced{The number of required frequencies is not the only
important point for reducing computation time. Another important aspect is the computation grid, as t}{T}he required \deleted{computation} grids
...
that solves Maxwell's equations in the time domain. \added{Our recommendations
for speeding up the Fourier transform for expensive kernels are independent of
...
any solver that provides sufficient accurate and precise frequency-domain
...
compute accurate results over a wide range of frequencies (times) to obtain
time-domain (frequency-domain) responses. In our case the solver uses a
regular, stretched grid and computes the electric fields in the frequency
...
$z$-direction\added{,} and \replaced{it equally applies}{similarly} for very
...
well for dispersive media where the model parameters are frequency dependent,
such as is the case in induced polarization. To test this we use the Cole-Cole
model \mbox{\citep[CCM, ][]{JCP.41.Cole}} which, written in terms of
...
naturally no different than modelling a non frequency-dependent model. However,
the interesting point is to see how the Fourier transform behaves to obtain
time-domain data from frequency-domain responses. As an example we modelled the
response for a land case, air above a halfspace of 1\,S/m. Within the halfspace
is a dispersive, 100\,m thick layer at a depth of 300\,m, with
$\sigma_0=1.0\,$S/m and $\sigma_\infty=1.25\,$S/m; $\tau=1$, and we vary
$c$ from 0 (no IP) to 1.0, 0.75, 0.5, and 0.25. The x-directed step-off source
and the receiver at an inline offset of 900\,m are place at the surface. The
real and imaginary frequency-domain responses for this model are shown in the
left column of Figure~\ref{fig:CCM}. It can be seen that the effect of the
IP layer in the frequency domain is that the real part starts to deviate from
the DC value at lower frequencies. The imaginary part decays slower towards
zero when decreasing the frequency, and it deviates from a linear function of 
frequency (on log-log scale) at lower frequencies as well. This indicates that
we need lower frequency content for the Fourier transform the smaller $c$
becomes.}
%
\begin{figure}
  \centering
...
values only 27 frequencies have to be computed, instead of 747 frequencies
